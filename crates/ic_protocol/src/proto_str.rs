...
pub const IC_CMD_GET_NODEID: &str = "get nodeid";
/// Reply to [`IC_CMD_GET_NODEID`]. Verify: `mgmapi.cpp:3295`.
pub const IC_REPLY_GET_NODEID: &str = "get nodeid reply";
...
pub const IC_CMD_GET_VERSION: &str = "get version";
/// Reply to [`IC_CMD_GET_VERSION`]. Note that it is not the command plus
/// " reply" like the others. Verify: `mgmapi.cpp:3692`.
pub const IC_REPLY_GET_VERSION: &str = "version";

/// Ask for the state of every node in the cluster.
/// Verify: `mgmapi.cpp:1212`.
pub const IC_CMD_GET_STATUS: &str = "get status";
/// Reply to [`IC_CMD_GET_STATUS`], again not the command plus " reply".
/// Verify: `mgmapi.cpp:1237`.
pub const IC_REPLY_GET_STATUS: &str = "node status";
...
pub const IC_CMD_GET_CONNECTION_PARAMETER: &str = "get connection parameter";
/// Reply to [`IC_CMD_GET_CONNECTION_PARAMETER`].
/// Verify: `mgmapi.cpp:3560`.
pub const IC_REPLY_GET_CONNECTION_PARAMETER: &str =
...
pub const IC_CMD_TRANSPORTER_CONNECT: &str = "transporter connect";
...
pub const IC_ARG_VERSION: &str = "version";
/// What kind of node we are; see [`IC_NODE_TYPE_API`].
pub const IC_ARG_NODETYPE: &str = "nodetype";
/// The node id we want, or 0 to let the management server choose.
pub const IC_ARG_NODEID: &str = "nodeid";
/// User name, which the protocol carries but does not check.
pub const IC_ARG_USER: &str = "user";
/// Password, likewise unchecked.
pub const IC_ARG_PASSWORD: &str = "password";
/// Public key, likewise unchecked.
pub const IC_ARG_PUBLIC_KEY: &str = "public key";
/// Our byte order: [`IC_ENDIAN_BIG`] or [`IC_ENDIAN_LITTLE`].
pub const IC_ARG_ENDIAN: &str = "endian";
/// An optional name for this connection, shown in the cluster log.
pub const IC_ARG_NAME: &str = "name";
/// Whether the management server should log events about us.
pub const IC_ARG_LOG_EVENT: &str = "log_event";
/// The node the configuration is wanted for, sent with `get config_v2`.
pub const IC_ARG_NODE: &str = "node";
/// Ask for the configuration as another node sees it.
pub const IC_ARG_FROM_NODE: &str = "from_node";
/// The two ends of a connection, for a connection parameter.
pub const IC_ARG_NODE1: &str = "node1";
/// See [`IC_ARG_NODE1`].
pub const IC_ARG_NODE2: &str = "node2";
/// Which parameter of the connection is wanted.
pub const IC_ARG_PARAM: &str = "param";
/// The value of the parameter.
pub const IC_ARG_VALUE: &str = "value";

/// Whether the command succeeded; [`IC_RESULT_OK`] or a message.
pub const IC_ARG_RESULT: &str = "result";
/// A numeric error code in a reply that failed.
pub const IC_ARG_ERROR_CODE: &str = "error_code";
/// Length in bytes of the blob that follows a reply.
pub const IC_ARG_CONTENT_LENGTH: &str = "Content-Length";
/// What the blob is; always [`IC_CONTENT_TYPE_CONFIG`].
pub const IC_ARG_CONTENT_TYPE: &str = "Content-Type";
/// How the blob is encoded; always [`IC_CONTENT_ENCODING_BASE64`].
pub const IC_ARG_CONTENT_ENCODING: &str = "Content-Transfer-Encoding";
...
/// A data node.
pub const IC_NODE_TYPE_DB: u32 = 0;
/// An API node, which is what this library is.
pub const IC_NODE_TYPE_API: u32 = 1;
/// A management server.
pub const IC_NODE_TYPE_MGM: u32 = 2;
...
mod tests {
  ...
  fn commands_and_replies_pair_up() {
    ...
    assert_eq!(IC_REPLY_GET_NODEID, format!("{} reply", IC_CMD_GET_NODEID));
    assert_eq!(IC_REPLY_GET_CONFIG, format!("{} reply", IC_CMD_GET_CONFIG));
    assert_eq!(
      IC_REPLY_GET_CONNECTION_PARAMETER,
      format!("{} reply", IC_CMD_GET_CONNECTION_PARAMETER)
    );
    assert_ne!(
      IC_REPLY_GET_CONFIG,
      format!("{} reply", IC_CMD_GET_CONFIG_V2)
    );
  }
  ...
  fn nothing_carries_stray_spaces() {
    let all = [
      ...
      IC_CMD_TRANSPORTER_CONNECT,
      IC_ARG_VERSION,
      IC_ARG_NODETYPE,
      IC_ARG_NODEID,
      IC_ARG_CONTENT_LENGTH,
      IC_ARG_CONTENT_TYPE,
      IC_ARG_CONTENT_ENCODING,
      IC_RESULT_OK,
  ...
  fn node_types_are_distinct() {
    assert_ne!(IC_NODE_TYPE_DB, IC_NODE_TYPE_API);
    assert_ne!(IC_NODE_TYPE_API, IC_NODE_TYPE_MGM);
    assert_eq!(IC_NODE_TYPE_API, 1);
  }
}
